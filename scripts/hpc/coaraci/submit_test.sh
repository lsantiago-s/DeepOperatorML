...
usage() {
  cat <<'EOF'
Usage:
  scripts/hpc/coaraci/submit_test.sh [options]
...
Options:
  ...
  --qos <name>                       Slurm QoS
  --job-name <name>                  Job name (default: deepop-test-<queue>)
...
JOB_SCRIPT="$(mktemp "/tmp/deepop_test_${QUEUE}_XXXXXX")"
cat > "${JOB_SCRIPT}" <<EOF
#!/bin/bash
set -euo pipefail
cd "${REPO_ROOT}"
