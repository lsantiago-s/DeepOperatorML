...
def _get_integration_timing(data_cfg: DataConfig) -> dict[str, float | None]:
    metadata_path = Path(data_cfg.raw_metadata_path)
    if not metadata_path.exists():
        return {"integration_total_s": None, "integration_per_sample_s": None}

    with open(metadata_path, "r", encoding="utf-8") as f:
        raw_meta = yaml.safe_load(f) or {}

    timing_breakdown = raw_meta.get("timing_breakdown", {})
    total = _safe_float(timing_breakdown.get("integration_total_s"))
    per_sample = _safe_float(timing_breakdown.get("integration_per_sample_s"))
    ...
    if total is None:
        ...
        if runtime_ms is not None:
            ...
            # older metadata stored seconds under "runtime_ms".
    ...
    return {
        "integration_total_s": total,
        "integration_per_sample_s": per_sample,
    }
...
def _plot_error_vs_delta(
    delta_test: np.ndarray,
    method_errors: dict[str, np.ndarray],
    output_path: Path,
) -> None:
    order = np.argsort(delta_test)
    delta_sorted = delta_test[order]

    fig, ax = plt.subplots(1, 1, figsize=(8, 4.5), constrained_layout=True)
    for method_name, errs in method_errors.items():
        ax.plot(delta_sorted, errs[order], lw=1.3, label=method_name)
    ax.set_xlabel("delta")
    ax.set_ylabel("Relative L2 error")
    ax.set_title("Error versus normalized frequency (delta)")
    ax.grid(alpha=0.25)
    ax.legend()
    fig.savefig(output_path, dpi=180)
    plt.close(fig)
...
def _plot_timing_comparison(timing_report: dict[str, Any], output_path: Path) -> None:
    integ = timing_report["integration"].get("per_sample_s")
    infer = timing_report["inference"].get("per_sample_s")
    if integ is None or infer is None:
        return

    fig, ax = plt.subplots(1, 1, figsize=(7, 4), constrained_layout=True)
    labels = ["Integration", "ML Inference"]
    values = [integ, infer]
...
def _save_performance_and_timing_reports(
    ...
    report = {
        "scope": "dynamic Green's function surrogate for fixed material (paper-aligned delta parameterization)",
    ...
    delta_test = np.asarray(data["xb_test"])[:, 0]
    _plot_method_error_distributions(
        method_errors=method_errors,
        output_path=performance_plots_dir / "error_distribution_by_method.png",
    )
    _plot_error_vs_delta(
        delta_test=delta_test,
        method_errors=method_errors,
        output_path=performance_plots_dir / "error_vs_delta.png",
    )

    integration_timing = _get_integration_timing(data_cfg=data_cfg)
    ...
    timing_report = {
        "integration": {
            "total_s": integration_timing["integration_total_s"],
            "per_sample_s": integration_timing["integration_per_sample_s"],
        },
        "inference": {
    ...
    integ_ps = integration_timing["integration_per_sample_s"]
    infer_ps = inference_per_sample
    if integ_ps is not None and infer_ps is not None:
        timing_report["speedups"]["per_sample_integration_over_inference"] = float(integ_ps / max(infer_ps, 1e-14))

    integ_total = integration_timing["integration_total_s"]
    if integ_total is not None and infer_ps is not None:
        n_total = int(data_cfg.data[data_cfg.features[0]].shape[0])
        infer_total_est = float(infer_ps * n_total)
        timing_report["inference"]["estimated_total_s_for_full_dataset"] = infer_total_est
        timing_report["speedups"]["estimated_full_dataset_integration_over_inference"] = float(
            integ_total / max(infer_total_est, 1e-14)
        )
